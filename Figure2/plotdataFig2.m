...
for unm=1:size(Pulse.Mua.FRpulse,1)  %%%% peak firing rate latency from pulse onset
     [maxS2_Mua(unm), posMaxS2_Mua(unm)]=max(Pulse.Mua.FRpulse(unm,10:30));
end

edges=0:1:20;
histogram(posMaxS2_Mua,edges,'FaceColor','r');hold on
set(gca,'Fontsize',30)
...
for uns=1:size(Pulse.Su.FRpulse,1)  %%%% peak firing rate latency from pulse onset
     [maxS2_Su(uns), posMaxS2_Su(uns)]=max(Pulse.Su.FRpulse(uns,10:30));
end

edges=0:1:20;
histogram(posMaxS2_Su,edges,'FaceColor','r');hold on
set(gca,'Fontsize',30)
